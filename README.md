...
#### We had a lot of fun participating in the IMC Prosperity 4 competition this year, and we are impressed with some of our results in the rounds. We find it especially inspiring that we had some notable improvements from last year, and still have improvements to learn for the future. We chose to approach our algorithms in a much safer manner, which overall seemed to reduce the maximum potential profit of our algorithms, but the profit that we did gain was hopefully safer and less likely to turn negative. This is opposed to last year's IMC Prosperity 3 competition, where our algorithms would occasionally finish with negative profit. Despite this, we definitely have more strategies to learn to incorporate in future algorithms, especially because we ended up not really trading some of this year's products at all.

#### In regards to our manual trading rounds, I think we did improve in many of our rounds (in comparison with previous years' rounds). For example, Round 3 was quite similar to last year (IMC Prosperity 3)'s Round 3, as both rounds essentially had the same premises of submitting two bids to potentially trade with the most counterparties and securing the most profit. In last year's Round 3, our second bid was less than the average second bid across all teams, so I don't know if our second bid in that round ended up securing profit. This year, our second bid was way more than the average second bid, which means we were able to trade with our second bid and potentially perform better in that round this year. We do still have a lot of room for improvement in the manual trading rounds as well, as exemplified by Round 5, in which we were too safe in many of our product allocations and chose to sell Obsidian Cutlery instead of buying it.

#### The biggest lesson we learned in the algorithm rounds is to work on prioritizing general strategy formation instead of only tweaking small values in each test submission. A thought process for only tweaking small values in each test submission is that, if we changed more than one aspect of the algorithm, we worried that it would be harder what change caused a potential difference in profit. However, doing so could be using more time than it saves, so it is definitely worth trying to balance small changes our more with general changes in strategy. We also didn't use an external backtester for the algorithm, so we wonder if using them in the future could have helped test our algorithm faster, and hence allow us to focus more on the general strategy of our algorithm.

#### Generally, the biggest lesson we learned in the manual trading rounds is to learn to balance our decision making between our intuition and our code. A lot of our success in the manual trading rounds stem from either our intuition, our code, or a combination of the two, so learning to better evaluate our code in comparison with our intuition could be a potential improvement for the future. This is best shown in Round 3 manual trading challenge, as our code for that round did output optimal second bids that were much closer to the actual optimal second bid. On the other hand, our code for the Round 4 manual trading challenge indicated us to buy almost every single option, which ended up not being the case.

#### Overall, we are satisfied with our participation in the IMC Prosperity 4 competition, and we had an enjoyable experience in the competition this year. We look forward to improving our skills and knowledge from what we have learned in this competition, and potentially participating in next year's IMC Prosperity 5 competition!
