...
class Strategy:
    ...
    def trade_emeralds(self, order_depth):
        ...
        for i in range(0, len(order_depth.sell_orders)):
            ...
            print(f"BUY EMERALDS: {str(-best_ask_amount)} x {self.acceptable_buy_price}")
            orders.append(Order(self.product_name, self.acceptable_buy_price, -best_ask_amount))

        for i in range(0, len(order_depth.buy_orders)):
            best_bid, best_bid_amount = list(order_depth.buy_orders.items())[i]

            print(f"SELL EMERALDS: {str(best_bid_amount)} x {self.acceptable_sell_price}")
            orders.append(Order(self.product_name, self.acceptable_sell_price, -best_bid_amount))
